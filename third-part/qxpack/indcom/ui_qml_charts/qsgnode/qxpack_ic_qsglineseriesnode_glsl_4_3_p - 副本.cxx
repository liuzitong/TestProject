#ifndef QXPACK_IC_QSGLINESERIESNODE_GLSL_4_3_P_CXX
#define QXPACK_IC_QSGLINESERIESNODE_GLSL_4_3_P_CXX

#include "qxpack_ic_qsglineseriesnode_glsl_4_3_p.hxx"
#include <QByteArray>
#include <QString>
...
#define SHADER_VEC_PROG_NAME  QStringLiteral("VecProg.IcQSGLineSeriesNodeGLSL4_3.QxPack")
#define SHADER_LST_PROG_NAME  QStringLiteral("LstProg.IcQSGLineSeriesNodeGLSL4_3.QxPack")
...
static const char *vtxTxt43_vec = // for ES 3.1 or Core 4.3
...
static const char *vtxTxt43_lst =
...
// ============================================================================
IcQSGLineSeriesNodeGLSL4_3 :: IcQSGLineSeriesNodeGLSL4_3(  QQuickItem *item )
         : IcQSGLineSeriesNode( item )
{
    ...
    if ( ! m_is_ogl_es ) {
        if ( ver.first == 4 ) {
            if ( ver.second < 3 ) { is_bad_ver = true; }
        } else if ( ver.first < 4 ){
            is_bad_ver = true;
        } else {  }
          is_bad_ver = true;
        ...
        if ( ver.first == 3 ) {
            if ( ver.second < 1 ) { is_bad_ver = true; }
        } else if ( ver.first < 3 ) {
            is_bad_ver = true;
        } else {  }
    }
    if ( is_bad_ver ) {
        qFatal("OpenGL version is not IcQSGLineSeriesNodeGLSL_4_3 required!");
    }
}
...
// dtor
// ============================================================================
IcQSGLineSeriesNodeGLSL4_3 :: ~IcQSGLineSeriesNodeGLSL4_3( )
{
    releaseResources();
}
...
// ============================================================================
auto     IcQSGLineSeriesNodeGLSL4_3 :: releaseResources() -> void
{
    if ( m_vec_prog_id != 0 ) {
...
// ============================================================================
auto     IcQSGLineSeriesNodeGLSL4_3 :: ensureUBO( bool sw ) -> void
{
    ...
    if ( ! sw  ) {
        ...
        if ( m_vec_prog_id != 0 && m_lst_prog_id != 0 ) {
            auto uni_idx = glf->glGetUniformBlockIndex( m_vec_prog_id, "UniValues" );
            GLint vec_uni_sz = 0;
            glf->glGetActiveUniformBlockiv( m_vec_prog_id, uni_idx, GL_UNIFORM_BLOCK_DATA_SIZE, &vec_uni_sz );
            uni_idx = glf->glGetUniformBlockIndex( m_lst_prog_id, "UniValues" );
            GLint lst_uni_sz = 0;
            glf->glGetActiveUniformBlockiv( m_lst_prog_id, uni_idx, GL_UNIFORM_BLOCK_DATA_SIZE, &lst_uni_sz );
...
// ============================================================================
auto     IcQSGLineSeriesNodeGLSL4_3 :: ensureVBO( bool sw ) -> void
{
    auto glf = QOpenGLContext::currentContext()->extraFunctions();
...
// init resources
// ============================================================================
auto     IcQSGLineSeriesNodeGLSL4_3 :: initResoruces() -> void
{
    if ( m_is_init ) { return; }
    ...
    if ( m_vec_prog_id == 0 ) {
        if ( vtx_vec_dat.isEmpty()) { vtx_vec_dat = versionedShaderCode( vtxTxt43_vec, m_is_ogl_es ); }
        if ( frag_dat.isEmpty())    { frag_dat    = versionedShaderCode( frag43, m_is_ogl_es ); }
    }
    if ( m_lst_prog_id == 0 ) {
        if ( vtx_lst_dat.isEmpty()) { vtx_lst_dat = versionedShaderCode( vtxTxt43_lst, m_is_ogl_es ); }
        if ( frag_dat.isEmpty())    { frag_dat    = versionedShaderCode( frag43, m_is_ogl_es ); }
    }
...
// ============================================================================
auto     IcQSGLineSeriesNodeGLSL4_3 :: setupUniform() -> void
{
    auto glf = QOpenGLContext::currentContext()->extraFunctions();
...
// upload UBO buffer
// ============================================================================
auto     IcQSGLineSeriesNodeGLSL4_3 :: bindAndUploadUBO( const RenderState *state ) -> void
{
    auto glf = QOpenGLContext::currentContext()->extraFunctions();
    ...
    glf->glBindBuffer( GL_UNIFORM_BUFFER, m_ubo_id );
    glf->glBindBufferBase( GL_UNIFORM_BUFFER, 0, m_ubo_id );
...
// upload VBO buffer
// ============================================================================
auto     IcQSGLineSeriesNodeGLSL4_3 :: bindAndUploadVBO_Vec() -> void
{
...
auto     IcQSGLineSeriesNodeGLSL4_3 :: bindAndUploadVBO_Lst() -> void
{
...
//  render
// ============================================================================
auto     IcQSGLineSeriesNodeGLSL4_3 :: render( const RenderState *state ) -> void
{
...
// other functions
// ============================================================================
auto     IcQSGLineSeriesNodeGLSL4_3 :: changedStates() const -> StateFlags
{ return BlendState | StencilState | ScissorState | DepthState; }

auto     IcQSGLineSeriesNodeGLSL4_3 :: flags() const -> RenderingFlags
{ return BoundedRectRendering /*| DepthAwareRendering*/; }

auto     IcQSGLineSeriesNodeGLSL4_3 :: rect()  const -> QRectF
