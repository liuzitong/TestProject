...
private:    
    ...
    struct QXPACK_IC_API PrivShared {
        QXPACK_IC_DISABLE_COPY( PrivShared )
    public:
        typedef void ( *DelCb )( void* );
        IcAtomicInt  m_atom_val;
        void *m_t_ptr; DelCb  m_del_cb;
        explicit PrivShared() : m_atom_val(1), m_t_ptr(nullptr), m_del_cb(nullptr) { }
        virtual ~PrivShared(){ if ( m_del_cb != nullptr ){ m_del_cb( m_t_ptr ); }}
        inline intptr_t  addRef()   { return m_atom_val.add(1); }
        inline intptr_t  decRef()   { return m_atom_val.sub(1); }
        inline void*&    tPtrRef()  { return m_t_ptr;  }
...
protected:
    ...
    static PrivShared*  createSharedPtr();
...
public :
    ...
    void  setValue( const T& val )
    {
        *this = IcVariant();
        ...
        m_priv.u_data.v_sp = new PrivShared;
        m_priv.u_data.v_sp->tPtrRef()  = tmp;
        m_priv.u_data.v_sp->delCbRef() = []( void *obj ){ delete reinterpret_cast<T*>(obj); };
    ...
    {
        if ( isSharedObj() ) {
            return * static_cast<T*>( m_priv.u_data.v_sp->tPtrRef() );
        } else {
            return T();
        }
    }
