...
private:    
    ...
    struct PrivShared {
        ...
        explicit PrivShared(){ m_ref.store(1); }
        virtual ~PrivShared(){}
    };

    // the sub class of shared descriptor
    template <typename T>
    struct PrivSharedImpl : public PrivShared {
        T  m_obj;  explicit PrivSharedImpl( const T &obj ) : m_obj( obj ){}
    };
...
protected:
    ...
    {
        if ( isSharedObj() ) {
            if ( m_priv.u_data.v_sp->m_ref.fetch_sub(1) - 1 == 0 ) {
...
public :
    ...
    IcVariant &   operator = ( const IcVariant &other )
    {
        if ( this != &other ) {
            detachSharedPtr();
            m_priv = other.m_priv;
            if ( isSharedObj()) { m_priv.u_data.v_sp->m_ref.fetch_add(1); }
        }
        return *this;
    }
    ...
    {
        *this = IcVariant();
        m_priv.u_data.v_sp = new PrivSharedImpl<T>( val );
    ...
    {
        if ( isSharedObj() ) {
            PrivSharedImpl<T> *sp = static_cast<PrivSharedImpl<T>*>( m_priv.u_data.v_sp );
            return sp->m_obj;
        } else {
            return T();
        }
