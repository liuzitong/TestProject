...
AnalysisMethodSvc *AnalysisMethodSvc::getSingleton()
{
    if (singleton==nullptr)
    {
        singleton=new AnalysisMethodSvc();
    }
    return singleton;
}
...
void AnalysisMethodSvc::drawRoundCrossPixScale(int range, QImage &img)
{
    int scale=img.width()>400?1:2;
    QPainter painter(&img);
...
void AnalysisMethodSvc::drawGray(QVector<int> values, QVector<QPointF> locs, int range, int innerRange, int imageSize, QString filePath)
{
    QImage img=QImage({imageSize,imageSize}, QImage::Format_RGB32);
...
void AnalysisMethodSvc::drawPE(QVector<int> values, QVector<QPointF> locs, int range,int imageSize, QString filePath)
{
    QImage img=QImage({imageSize,imageSize}, QImage::Format_RGB32);
    img.fill(qRgb(255, 255, 255));
    QPainter painter(&img);
...
void AnalysisMethodSvc::drawScreening(QVector<int> values, QVector<QPointF> locs, int range, int imageSize, int OS_OD,QString filePath)
{
    QImage img=QImage({imageSize,imageSize}, QImage::Format_RGB32);
    img.fill(qRgb(255, 255, 255));
    drawPixScale(range,img);
...
void AnalysisMethodSvc::drawDynamic(QVector<QPointF> values, int imageSize, int range, QString filePath)
{
    QImage img=QImage({imageSize,imageSize}, QImage::Format_RGB32);
    img.fill(qRgb(255, 255, 255));
    drawPixScale(range,img);
...
void AnalysisMethodSvc::drawFixationDeviation(QVector<int> values, QString filePath)
{
    int scale=2;
...
QPoint AnalysisMethodSvc::convertDegLocToPixLoc(QPointF DegLoc,int range,QImage img)
{
...
//void AnalysisMethodSvc::dynamicAnalysis()
//{
